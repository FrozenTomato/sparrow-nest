...
Consider[1]=function()
	...
	then
		...
		then	
			...
			then
				return 0.65,WeakestEnemy:GetExtrapolatedLocation(CastPoint)  --action_desire between moderate and high
			end	
		end
	end
	...
	then
		...
		then
			...
			then
				return 0.6, locationAoE.targetloc  --action_desire between moderate and high
			end
		end
	end
...
Consider[2]=function()
	...
	--------------------------------------
	--protect myself
	if((HealthPercentage<=0.3 and #enemys>=1) or #enemys >=3)
	then
		for _,npcEnemy in pairs( enemys )
		do
	...
	then
		...
		then
			if(CreepHealth<=WeakestCreep:GetActualIncomingDamage(ability:GetLevel()*npcBot:GetBaseDamage()+npcBot:GetAttackDamage(),DAMAGE_TYPE_PHYSICAL) and GetUnitToUnitDistance(npcBot,WeakestCreep)<=300)
			then
...
--get total enemy numbers near target
...
Consider[4]=function()
	...
	then
		...
		then
			...
			local locationAoE2 = npcBot:FindAoELocation( true, false, locationAoE.targetloc, 0, Radius, 0, 0 )
			if ( locationAoE.count+locationAoE2.count >= 6 ) 
			then
				npcBot:Action_UseAbilityOnLocation( blink,locationAoE.targetloc )
				return 1  --change from 0
			end
...
function AbilityUsageThink()
	...
	if ( npcBot:IsUsingAbility() or npcBot:IsChanneling() or npcBot:IsSilenced() or DotaTime()<=-20)
	then 
		return
	end
	
	ComboMana=GetComboMana()+UltAbility:GetManaCost()
