...
function ConsiderGlyph()
    for i, building_id in pairs(Towers) do
		...
		if tower~=nil
		then	
			if tower:GetHealth() <=200 and tower:TimeSinceDamagedByAnyHero()+tower:TimeSinceDamagedByCreep() <= 5 --and AttackByEnemy
			then
...
function GetComboMana(AbilitiesReal)
	...
	return math.max(tempComboMana,150)  --change from 300
